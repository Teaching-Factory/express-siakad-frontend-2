...
const update = async () => {
    try {
        ...
        const id_tagihan_mahasiswa = route.params.id_tagihan_mahasiswa || route.query.id_tagihan_mahasiswa;
        ...
        const response = await axios.put(`${API_URL}/tagihan-mahasiswa/${id_tagihan_mahasiswa}/update`, payload, {
            headers: {
                Authorization: token
            }
        });
...
const getTagihanMahasiswa = async (id_tagihan_mahasiswa) => {
    try {
        Swal.fire({
            title: 'Loading...',
            html: 'Sedang Memuat Data',
        ...
        tagihanMahasiswa.value = data;
        console.log('respons : ', data);
        Swal.close();
    } catch (error) {
        Swal.fire('Error', 'Gagal memuat data tagihan mahasiswa.', 'error');
...
onMounted(() => {
    ...
    if (id_tagihan_mahasiswa) {
        isEdit.value = true;
        getTagihanMahasiswa(id_tagihan_mahasiswa);
    }
});
</script>
    ...
    <form @submit.prevent="update">
        <div class="card">
            <div class="card-body">
                <div class="row">
                    <div class="col-lg-4">
                        <h5><i class="pi pi-user me-2"></i>UPDATE TAGIHAN - {{ tagihanMahasiswa?.Mahasiswa?.nama_mahasiswa }}</h5>
                    </div>
