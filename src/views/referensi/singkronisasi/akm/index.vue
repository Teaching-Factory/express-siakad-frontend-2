...
const getAKM = async () => {
    try {
        ...
        // Menggunakan axios untuk GET request dengan query parameters
        const response = await getData(`perkuliahan-mahasiswa-sync/belum-singkron/by-filter?jenis_singkron=${jenis_singkron}`);
...
<template>
    <div class="card">
        <h5><i class="pi pi-user me-2"></i>Singkronisasi AKM ke Feeder</h5>
                ...
                <div class="col-12 col-md-6 col-lg-12">
                    ...
                    <p class="lh-1 text-small">
                        <ol>
                            ...
                            <li>Memilih jenis singkron untuk data AKM </li>
                            <li>Memilih data yang hendak disinkron pada Feeder dan menekan tombol sync.</li>
                            <li>Pengambian data terbaru dari Feeder dan lokal dapat melalui tombol <span> <a  href="/sync-akm-get"> Klik Disini !!</a></span></li>
                            <li>Menghapus data dari Feeder dan lokal dapat melalui tombol <span> <a  href="#"> Klik Disini !!</a></span></li></ol>
                    </p>
                </div>
            </div>
        </div>
            ...
            :value="AKM" v-model:selection="selectedAKM" :paginator="true" :rows="20" dataKey="id" :rowHover="true" showGridlines :first="first" @page="onPageChange">
                ...
                <Column filterField="nim" header="NIM" style="min-width: 10rem">
                    <template #body="{ data }">
                        <div class="flex align-items-center gap-2">
                            <span>
                                {{ data.PerkuliahanMahasiswa?.Mahasiswa?.nim || '-'}}
                            </span>
                        </div>
                    </template>
                </Column>
                ...
                <Column filterField="nama_mahasiswa" header="Nama Mahasiswa" style="min-width: 15rem">
                    <template #body="{ data }">
                        <div class="flex align-items-center gap-2">
                            <span>
                                {{ data.PerkuliahanMahasiswa?.Mahasiswa?.nama_mahasiswa || '-'}}
                            </span>
                        </div>
                    </template>
                </Column>
                ...
                <Column filterField="angkatan" header="Angkatan" style="min-width: 8rem">
                    <template #body="{ data }">
                        <span>
                            {{ data.PerkuliahanMahasiswa?.angkatan || '-'}}
                            </span>
                    </template>
                </Column>
